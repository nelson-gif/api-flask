...
def search_by_title(title):
    ...
    for book in books:
        ...
        if temporary_book['title'].upper() == title.upper():
            books_list.append(temporary_book)
    return jsonify(books_list), 200

@app.route('/search/author/<author_name>', methods=['GET'])
def search_by_author(author_name):
    books = Book.query.join(Author).add_columns(Book.book_isbn, Book.genre_id, Book.title,
                Book.stock, Book.price, Book.pages, Book.publication_date,
                            Book.stock_added_date, Author.name).all()
    result_list = []

    for book in books:
        if book.name.upper() == author_name.upper():
            temporary_book = {
                'book_isbn': book.book_isbn,
                'genre_id': f'{Genre.query.get(book.genre_id)}',
                'author': book.name,
                'title': book.title,
                'stock': book.stock,
                'price': book.price,
            ...
            result_list.append(temporary_book)
    return jsonify(result_list), 200

@app.route('/search/genre/<genre_name>', methods=['GET'])
def search_by_genre(genre_name):
    books = Book.query.join(Genre).add_columns(Book.book_isbn, Book.author_id, Book.title,
                Book.stock, Book.price, Book.pages, Book.publication_date,
                            Book.stock_added_date, Genre.genre).all()
    result_list = []
    ...
    for book in books:
        ...
        if book.genre.upper() == genre_name.upper():
            result_list.append(temporary_book)
    return jsonify(result_list), 200

@app.route('/search/isbn/<int:isbn>', methods=['GET'])
...
def create_author():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'message' : 'an error occured while creating the author',
                        'error' : f'{e}'}), 400
    return jsonify({'message' : 'author was added successfully',
                    'author added' : f'{author}'}), 201
